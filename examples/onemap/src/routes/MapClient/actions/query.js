 ...
 * @Last Modified by: 史涛
 * @Last Modified time: 2020-10-10 11:28:28
 */
...
function setSelectPOIItem(item) {
    return {
        type: SELECT_POI,
        item:item
    };
...
function query(key) {
    ...
    return (dispatch, getState) => {
        ...
        return axios.get(mapConfigJson.tdtserverurl + "/search", {
            ...
            params: {
              postStr: assign({},{
                ...
                mapBound:[bounds._ne.lng,bounds._ne.lat,bounds._sw.lng,bounds._sw.lat].join(","),
                level: 11,
                queryType: query.nearbytitle?3:2,
                count: query.page,
                start: (query.pageindex - 1) * query.page
              },query.nearbytitle&&{pointLonlat:query.nearbypoint,
                queryRadius:2000}),
              type: "query",
              tk: mapConfigJson.tdttk
            }
          }).then((response) => {
            ...
            if(response.data.pois){
                ...
                dispatch(querySearchResponse(response.data.pois,Number(response.data.count)));
